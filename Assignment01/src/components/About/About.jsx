import React from 'react'
import './About.scss'
import img2 from './bg-2.png'
import img3 from './bg-3.png'
import { FaPlay } from "react-icons/fa";
import ellipse1 from './ellipse1.jpg'

const About = () => {
  return (
    <div className='about-wrapper'>
      <h2>About Us</h2>
      <div className="about-first">
        <div className="about-first-left">
          <h3>WE PRIDE OURSELF ON</h3>
          <h1> <span className='orange-text'>Our authentic momo recipes </span>passed down through generations</h1 >
        </div>
        <div className='about-first-right'>
          <div className='ellipses-top'>
            <div className='ellipses'>
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
            </div>
            <div className='ellipses'>
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
            </div>
            <div className='ellipses'>
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
            </div>
            <div className='ellipses'>
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
            </div>
            <div className='ellipses'>
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
            </div>
            <div className='ellipses'>
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
            </div>
          </div>
          <div className="about-first-image">
            <div className="about-bg"></div>
            <img src={img2} alt="img2" />
          </div>
          <div className='ellipses-bottom'>
            <div className='ellipses'>
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
            </div>
            <div className='ellipses'>
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
            </div>
            <div className='ellipses'>
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
            </div>
            <div className='ellipses'>
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
            </div>
            <div className='ellipses'>
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
            </div>
            <div className='ellipses'>
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
              <img src={ellipse1} alt="Ellipse 1" />
            </div>
          </div>
        </div>
      </div>
      <div className="about-second">
        <img src={img3} alt="img3" />
        <div className='about-second-content'>
          <h1>Process behind the making</h1>
          <p>See how we make momos that you like from only the best ingredients</p>
          <div className='btn'>
            <button>
              <span className='btn-icon'><FaPlay /></span>
              <span className='btn-text'>Watch the video</span>
            </button>
          </div>
        </div>
      </div>
    </div>
  )
}

export default About